public class RolesAndPermissions{
    ...
    public int isPrivilegedUserOrNot(String username, String password) {
        int isFound = -1;
        for (int i = 0; i < User.adminCredentials.length; i++) {
            if (username.equals(User.adminCredentials[i][0])) {
                if (password.equals(User.adminCredentials[i][1])) {
                    isFound = i;
                    break;
                }
            }
    ...
    public String isPassengerRegistered(String email, String password) {
        String isFound = "0";
        for (Customer c : Customer.getCustomersCollection()) {
